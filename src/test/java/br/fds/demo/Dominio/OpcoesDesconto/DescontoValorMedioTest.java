...
public class DescontoValorMedioTest {
    ...
    public void valorMedioUltimas3ComprasAcima10mil() {
        ...
        Assertions.assertEquals(0.1f, implDesconto.calcularDesconto(pedidoTeste));
    }

@Test
    public void valorMedioUltimas3ComprasAcima20mil() {
        Cliente cli = new Cliente("Joe Doe");   
        Produto prodTest = new Produto("Teste", 20000.0, 100, 10, 50);
        for (int i=0; i<3; i++) {
            Pedido ped = new Pedido(cli);
            ped.addItem(new ItemPedido(2, prodTest));
            cli.addPedido(ped);
        ...
        cli.addPedido(pedidoTeste);
        System.out.println("AQUI" + cli.getPedidos().size());
        Assertions.assertEquals(0.15f, implDesconto.calcularDesconto(pedidoTeste));
    }
